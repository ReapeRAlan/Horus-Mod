...
The test-RC preflight refuses an unmerged branch, a non-annotated or mismatched tag, failed automated/headless-soak gates, connected Windows/Linux gates that are anything other than explicitly `PENDING`, missing experimental warnings, divergent manifest/checksums, an existing release, or any state in which `v1.4.3` is no longer GitHub Latest. Only after explicit publication approval, re-run it with `-CreateDraft`. That switch creates and verifies an unpublished prerelease draft; it never publishes the release or replaces an existing tag or asset. A stable `v2.0.0` release remains blocked until the complete connected matrix passes.
