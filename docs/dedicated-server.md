...
- Accepted commands and bounded authenticated rejection records are written as daily JSONL under `BepInEx/config/HorusMod/audit`; the default retention is 14 days and pruning is reevaluated daily. Rejection auditing has an independent per-SteamID rate limit. UDP, invalid-session, malformed, and excess rejection traffic is never allowed to amplify the audit file.
